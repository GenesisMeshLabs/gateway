...
await check('forged signature rejected', '/verify', {token: operator, method: 'POST', body: {certificate: {...certificate, cert_id: 'security-forgery', signatures: certificate.signatures.map(s => ({...s, sig: Buffer.alloc(64).toString('base64')}))}}, expected: 200, verify: text => { const d = JSON.parse(text); return d.trusted === false && JSON.stringify(d.reasons).includes('BadSignature'); }});
