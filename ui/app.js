...
async function refresh(){try{const [health,ready,api]=await Promise.all(['/health','/ready','/api'].map(p=>request(p)));$('health-value').textContent=health.response.ok?'Operational':'Unavailable';$('ready-value').textContent=ready.response.ok?'Ready':'Not ready';$('health-detail').textContent='Checked '+new Date().toLocaleTimeString();$('connection').textContent=health.response.ok?'Gateway connected':'Gateway unavailable';$('connection-dot').classList.toggle('bad',!health.response.ok);$('version').textContent='v'+(api.data.version||'?');const c=api.data.security_capabilities||{};$('platform-status').textContent='Durable state: '+(c.durable_crl_state?'on':'off')+' | Audit: '+(c.durable_audit?'on':'off')+' | Shared quotas: '+(c.shared_quotas?'on':'off')+' | OIDC: '+(c.oidc?'on':'off');}catch{$('health-value').textContent='Unavailable';$('ready-value').textContent='Unknown';$('connection').textContent='Connection unavailable';}}
